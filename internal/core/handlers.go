...
func (d *Daemon) handlePacket(dev *Device, l *lan.Link, p *proto.Packet) {
	...
	case proto.TypeMousepad:
		d.handleMousepad(dev, l, p)
	case proto.TypeMousepadKeyboard:
		var body struct {
			State bool `json:"state"`
		}
		if p.Decode(&body) == nil {
			d.mu.Lock()
			dev.keyboard = body.State
			d.mu.Unlock()
			d.markDirty()
		}
	case proto.TypeMousepadEcho:
	case proto.TypePresenter:
		d.handlePresenter(p)
	case proto.TypeSftp:
		d.handleSftp(dev, p)
...
// handleMousepad moves the pointer and types keys for a phone touchpad.
func (d *Daemon) handleMousepad(dev *Device, l *lan.Link, p *proto.Packet) {
	d.mu.Lock()
	allowed := d.cfg.ReceiveInput
	d.mu.Unlock()
	if !allowed {
		return
	}
	...
	case b.Scroll:
		in.Scroll(b.DX, b.DY)
	case b.SpecialKey > 0:
		in.SpecialKey(b.SpecialKey, b.Shift, b.Ctrl, b.Alt, b.Super)
	case b.Key != "":
		if b.Ctrl || b.Alt || b.Super {
			in.KeyWithMods(b.Key, b.Shift, b.Ctrl, b.Alt, b.Super)
		} else {
			in.TypeText(b.Key)
		}
	case b.DX != 0 || b.DY != 0:
	...
	if b.SendAck {
		...
		body["isAck"] = true
		delete(body, "sendAck")
		_ = l.Send(proto.New(proto.TypeMousepadEcho, body))
	}
}

// handlePresenter moves the pointer for the Flux presentation remote. The
// dx and dy values are in pixels.
func (d *Daemon) handlePresenter(p *proto.Packet) {
	d.mu.Lock()
	allowed := d.cfg.ReceiveInput
	d.mu.Unlock()
	if !allowed {
	...
	if p.Decode(&b) != nil || b.Stop {
		return
	}
	if in := d.inputDevice(); in != nil {
		in.Move(b.DX, b.DY)
	}
}

// inputDevice creates the uinput device on first use. It reports a
// permission problem once.
func (d *Daemon) inputDevice() *desktop.Input {
	d.mu.Lock()
	defer d.mu.Unlock()
	if d.input != nil || d.inputErr != nil {
		return d.input
	}
	in, err := desktop.NewInput()
	if err != nil {
		d.inputErr = err
		d.logf("remote input: %v", err)
		go d.notify(desktop.Notification{AppName: "Flux", Title: "Remote input is not available", Body: err.Error()})
		return nil
	}
	d.input = in
	return in
