...
The feature is off until you turn it on. The phone needs a fingerprint in
its settings. To turn it on for `sudo`:

1. Open Flux on the phone, and check that the computer is connected.
2. Run the setup from your own user:
...
5. In a new terminal, run `sudo -k` and then `sudo true`. The terminal
   shows `Approve on <phone>, or wait for the password prompt.`, and the
   phone shows the request.

The setup checks that root owns the helper. It adds this line before the
first `auth` rule of `/etc/pam.d/sudo`, and keeps a copy of the file in
`/etc/flux/approve/pam-backup/`:

```text
auth sufficient pam_exec.so quiet stdout /usr/lib/flux/flux-approve
```

To turn it on for polkit or hyprlock too, name the services. The setup
copies the vendor file of polkit to `/etc/pam.d/polkit-1` first:

```sh
sudo flux approve enable polkit-1 hyprlock
sudo flux approve disable            # remove the line from all PAM files
```
...
The Omarchy lock screen uses its own PAM services, and Flux does not change
them. Flux never changes `sshd` or `login`, and the helper refuses `sshd`.

```sh
flux approve              # the enrolled phone and the PAM files that use it
sudo flux approve remove  # delete the key file and remove the PAM lines
```
